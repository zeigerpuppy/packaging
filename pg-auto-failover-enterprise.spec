...
Source0:	https://github.com/citusdata/pg-auto-failover-enterprise/archive/v1.3.tar.gz
...
# Check if -Wl,-pie support exists in libpgport.a
# Source: https://stackoverflow.com/a/1351771/2570866
if ! readelf --relocs %{pginstdir}/lib/libpgport.a | egrep '(GOT|PLT|JU?MP_SLOT)'; then
    if [ -z "${UNENCRYPTED_PACKAGE:-}" ]; then
        echo ERROR: Postgres is not compiled with -fPIC so -Wl,pie cannot be used. This is required for Microsoft packages
        exit 1
    else
